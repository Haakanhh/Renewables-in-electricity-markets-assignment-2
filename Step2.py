...
# Generate scenarios
Load_profiles = uf.Load_profile_generation(random_state=seed, Profiles=300, P_max=600, P_min=220, P_delta=35, plot=True)
...
idx = [sample_rng.choice(Load_profiles.shape[0], size=n_insample_profiles, replace=False) for _ in range(3)][-1] # For some reason we ran this line of code 3 times, and now we are sticking with it to keep the same results
...
# ALSO-X calculations for out-of-sample point violations
...
# CVaR calculations for out-of-sample point violations
...
# Profile-level violations (a profile violates if more than 6 minutes violate)
...
# Count profiles with violations
...
# Calculate share of profiles with violations
...
# Loop over epsilon values, solve ALSO-X, and store results
...
# Add mean shortfall for each Pxx requirement
